...
Blue Arrow specific OFSCI map (worker name and job description added to product description)
...
******************************************************************************************
	         |              	|	                                                            
02/03/2010 | A Barber  	| Case 3392: Use invoice number as order number.                                              
***************************************************************************************-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:script="http://mycompany.com/mynamespace" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
							...
							<InvoiceDetail>
								<xsl:for-each select="/Invoice/InvoiceItem">
									...
									<InvoiceLine>
										...
										<!-- 1587 Put together a product description -->										
										<ProductDescription>
											<xsl:value-of select="/Invoice/CSGExtension/WorkerName"/>
											<xsl:text>, </xsl:text>
											<xsl:value-of select="/Invoice/CSGExtension/JobDesc"/>
											<xsl:text>, w/c </xsl:text>
											<xsl:value-of select="script:getWeekBeingBilled(substring-before(/Invoice/TaxPointDateTime, 'T'))"/>
										</ProductDescription>
												...
												<xsl:otherwise>
													<xsl:choose>
														<xsl:when test="/Invoice/TaxPointDateTime !=''">
															<xsl:choose>
																<xsl:when test="translate(substring-before(/Invoice/TaxPointDateTime, 'T'),'-','')  &gt;= translate('2011-01-04','-','') ">
														...
														<xsl:when test="/Invoice/InvoiceDocumentDetails/InvoiceDocumentDate !=''">
															<xsl:choose>
																<xsl:when test="translate(substring-before(/Invoice/InvoiceDocumentDetails/InvoiceDocumentDate, 'T'),'-','')  &gt;= translate('2011-01-04','-','')">
																	<xsl:value-of select="format-number($NewTaxRate, '0.00')"/>
																</xsl:when>
	...
	Function getWeekBeingBilled(docDate)
	
		Dim dayOfWeek 
		Dim daysSinceLastMonday
		
		dayOfWeek = Weekday(docDate)
		daysSinceLastMonday = (dayOfWeek - 2) + 7
		
		getWeekBeingBilled = FormatDateTime(DateAdd("d",-1 * daysSinceLastMonday ,docDate), vbGeneralDate)

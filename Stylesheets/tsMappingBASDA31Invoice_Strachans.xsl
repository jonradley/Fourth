...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:script="http://mycompany.com/mynamespace" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template name="extractSBR">
		<xsl:param name="rawValue"/>
		
		<xsl:variable name="contractCodeInBrackets">		
			<xsl:call-template name="getRightHandWord">
				<xsl:with-param name="text" select="$rawValue"/>
			</xsl:call-template>		
		</xsl:variable>
		
		<xsl:value-of select="translate($contractCodeInBrackets,'()','')"/>	
		
	</xsl:template>
	
	<xsl:template name="getRightHandWord">
		<xsl:param name="text"/>
		
		<xsl:choose>
			<xsl:when test="$text='' or substring($text,string-length($text),1)=' ' "/><!-- This is the base case -->
			<xsl:otherwise>

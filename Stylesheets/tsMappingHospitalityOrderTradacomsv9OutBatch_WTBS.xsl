...
==========================================================================================
 23/02/2009	| Moty Dimant	| Created module. Adapted from Scottish & Newcastle order mapper
=======================================================================================-->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/BatchRoot[PurchaseOrder]">
		...
		<xsl:for-each select="PurchaseOrder">
			...
			<xsl:text>1+++</xsl:text>
			<xsl:choose>
				<xsl:when test="PurchaseOrderHeader/HeaderExtraData/OrderType">
					<xsl:value-of select="PurchaseOrderHeader/HeaderExtraData/OrderType"/>
				</xsl:when>
				<xsl:otherwise>FU</xsl:otherwise>
			</xsl:choose>
			<xsl:value-of select="$sRecordSep"/>
			...
			<xsl:text>MTR=</xsl:text>
				<xsl:value-of select="7 + (2 * count(PurchaseOrderDetail/PurchaseOrderLine))"/>
			<xsl:value-of select="$sRecordSep"/>

...
<xsl:output method="text" encoding="utf-8"/>
	...
	<!--xsl:template match="/Invoice[InvoiceDetail/InvoiceLine/LineExtraData[StockSystemID='CW' and IsStockProduct[.='true' or .='1']]]"-->
...
-->
		...
		<!-- From section 4.1.1.3	 of Caterwide Integration Functional Specification v1.2
	...
	<xsl:template name="msTrim">
		...
		<xsl:choose>
			<xsl:when test="starts-with($vsText,' ')">
				<xsl:call-template name="msTrim">
					<xsl:with-param name="vsText" select="substring($vsText,2)"/>
				</xsl:call-template>
			</xsl:when>
			
			<xsl:when test="substring($vsText,string-length($vsText),1)">
				<xsl:call-template name="msTrim">
					<xsl:with-param name="vsText" select="substring($vsText,1,string-length($vsText)-1)"/>
				</xsl:call-template>
			</xsl:when>
			
			<xsl:otherwise>
				<xsl:value-of select="$vsText"/>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template name="msPad">
		...
		<xsl:choose>
			<xsl:when test="string-length($vsText) &gt;= 201"><xsl:value-of select="substring($vsText,1,201)"/></xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="msPad"><xsl:with-param name="vsText" select="concat($vsText,' ')"/></xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>

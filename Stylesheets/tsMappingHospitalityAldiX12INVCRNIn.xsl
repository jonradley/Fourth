...
<!--======================================================================================
 Overview
ANSI X12 810 V5 - outbound invoice mapper  
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/Invoice">
		...
		<xsl:value-of select="js:msPad(string(InvoiceHeader/Supplier/SuppliersLocationID/SuppliersCode),15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- I07 Interchange Receiver ID -->
		<xsl:text>0041498000097</xsl:text>
		<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:value-of select="format-number(InvoiceHeader/BatchInformation/FileGenerationNumber,'000000000')"/>
		<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Supplier/SuppliersLocationID/SuppliersCode),15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 124 Application Receiver's Code -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersLocationID/SuppliersCode),15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 373 Date -->
		<xsl:value-of select="translate(InvoiceHeader/BatchInformation/SendersTransmissionDate,'-','')"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 337 Time -->
		<xsl:value-of select="js:msTruncate(string(substring-after(InvoiceHeader/BatchInformation/SendersTransmissionDate,':')), 4)"/>
		<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 93 Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersName), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine1), 15)"/>
		<xsl:value-of select="$sRecordSep"/>
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 19 City Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine3), 30)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 156 State or Province Code - NOT USED -->
		<xsl:value-of select="$sFieldSep"/>
		<!-- 116 Postal Code -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/PostCode), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 26 Country Code -->
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 93 Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Supplier/SuppliersName), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 66 Identification Code Qualifier -->
		<xsl:text>ZZ</xsl:text>
		<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 166 Address Information -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Supplier/SuppliersAddress/AddressLine1), 15)"/>
		<xsl:value-of select="$sRecordSep"/>
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 19 City Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Supplier/SuppliersAddress/AddressLine3), 30)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 156 State or Province Code - NOT USED -->
		<xsl:value-of select="$sFieldSep"/>
		<!-- 116 Postal Code -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Supplier/SuppliersAddress/PostCode), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 26 Country Code -->
		<xsl:text>UK</xsl:text>
		<xsl:value-of select="$sRecordSep"/>
		...
		<xsl:value-of select="$sFieldSep"/>
		<!-- 93 Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToName), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 66 Identification Code Qualifier -->
		<xsl:text>ZZ</xsl:text>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 67 Identification Code -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToLocationID/SuppliersCode), 15)"/>
		<xsl:value-of select="$sRecordSep"/>
		<!-- N3 -->
		<xsl:text>N3</xsl:text>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 166 Address Information -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine1), 15)"/>
		<xsl:value-of select="$sRecordSep"/>
		<!-- N4 -->
		<xsl:text>N4</xsl:text>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 19 City Name -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine3), 30)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 156 State or Province Code - NOT USED -->
		<xsl:value-of select="$sFieldSep"/>
		<!-- 116 Postal Code -->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/PostCode), 15)"/>
		<xsl:value-of select="$sFieldSep"/>
		<!-- 26 Country Code -->
		<xsl:text>UK</xsl:text>
		<xsl:value-of select="$sRecordSep"/>
		
		<!-- ITD segment is optional so is not added -->
		...
		<xsl:for-each select="InvoiceDetail/InvoiceLine">
			...
			<xsl:value-of select="format-number(number(InvoicedQuantity), '##.000')"/>
			<xsl:value-of select="$sFieldSep"/>
			<!-- 355 Unit or Basis for Measurement Code -->
			<xsl:choose>
				<xsl:when test="InvoicedQuantity/@UnitOfMeasure='EA'"><xsl:text>PC</xsl:text></xsl:when>
				<xsl:when test="InvoicedQuantity/@UnitOfMeasure='CS'"><xsl:text>CA</xsl:text></xsl:when>
				<xsl:when test="InvoicedQuantity/@UnitOfMeasure='PND'"><xsl:text>PN</xsl:text></xsl:when>
				<xsl:when test="InvoicedQuantity/@UnitOfMeasure='KG'"><xsl:text>KG</xsl:text></xsl:when>
			...
			<!-- TX1 - Tax Information -->
			<xsl:text>TX1</xsl:text>
			<xsl:value-of select="$sFieldSep"/>
			<!-- 963 Tax Type Code -->
			<xsl:text>VA</xsl:text>
			<xsl:value-of select="$sFieldSep"/>
			<!-- 782 Monetary Amount -->
			<xsl:value-of select="format-number(number(LineValueExclVAT) - number(UnitValueExclVAT), '#.00')"/>
			<xsl:value-of select="$sFieldSep"/>
			<!-- 954 Percentage as Decimal -->
			<xsl:value-of select="format-number(number(VATRate), '#.00')"/>
			<xsl:value-of select="$sFieldSep"/>
		...
		<xsl:for-each select="InvoiceTrailer/VATSubTotals/VATSubTotal">
			...
			<xsl:value-of select="$sFieldSep"/>
			<!-- 782 Monetary Amount -->
			<xsl:value-of select="format-number(number(DocumentTotalExclVATAtRate), '#.00')"/>
			<xsl:value-of select="$sRecordSep"/>
			...
			<xsl:value-of select="$sFieldSep"/>
			<!-- 782 Monetary Amount -->
			<xsl:value-of select="format-number(number(DocumentDiscountAtRate), '#.00')"/>
			<xsl:value-of select="$sRecordSep"/>	
		</xsl:for-each>
		...
		<xsl:value-of select="6 + 2 * number(InvoiceTrailer/NumberOfLines)"/>
		<xsl:value-of select="$sFieldSep"/>
		<xsl:text>0001</xsl:text>		
		<xsl:value-of select="$sRecordSep"/>

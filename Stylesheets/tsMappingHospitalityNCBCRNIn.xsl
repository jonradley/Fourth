...
Name			| Date			| Change
**********************************************************************
M Dimant		| 31/01/2014	| 7664 Created.
**********************************************************************
*******************************************************************-->
...
<BatchRoot>
	...
	<xsl:template match="TradeSimpleHeader">
		<TradeSimpleHeader>		
		<xsl:choose>
			<xsl:when test="./CreditNote/CreditNoteHeader/Buyer/BuyersLocationID/SuppliersCode='5027615900013'">
			...
			<xsl:when test="../CreditNoteHeader/Buyer/BuyersLocationID/SuppliersCode='5060166761103'">
				<SendersCodeForRecipient>
					<xsl:value-of select="../CreditNoteHeader/ShipTo/ShipToLocationID/SuppliersCode"/>
				</SendersCodeForRecipient>
				<SendersBranchReference>
					<xsl:value-of select="substring(../CreditNoteHeader/ShipTo/ShipToLocationID/SuppliersCode,8,1)"/>
				</SendersBranchReference>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="CreditNoteLine">
		...
		<CreditNoteLine>
			...
			<xsl:variable name="sUoM">
				<xsl:choose>
					<xsl:when test="CreditedQuantity/@UnitOfMeasure = 'KG' or string(./Measure/TotalMeasureIndicator) = 'KGM' ">KGM</xsl:when>
					<xsl:when test="CreditedQuantity/@UnitOfMeasure = 'KG'">KGM</xsl:when>
					<xsl:when test="CreditedQuantity/@UnitOfMeasure = 'EACH'">EA</xsl:when>
					<xsl:when test="CreditedQuantity/@UnitOfMeasure = 'CASE'">CS</xsl:when>
					<xsl:when test="CreditedQuantity/@UnitOfMeasure = 'DOZEN'">DZN</xsl:when>
			...
			<CreditedQuantity>				
				...
				<xsl:choose>
					<xsl:when test="substring-after(CreditedQuantity,' ')=0">
						<xsl:value-of select="format-number(substring-before(CreditedQuantity,' '),0000)"/>
					</xsl:when>
					<xsl:otherwise>						
						<xsl:value-of select="format-number((substring-after(CreditedQuantity,' ')) div 1000.0, '0.00##')"/>
					</xsl:otherwise>
				</xsl:choose>				
			</CreditedQuantity>

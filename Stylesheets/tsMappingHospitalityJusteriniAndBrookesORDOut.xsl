...
Outbound Purchase Order Stylesheet, based on Maginus CSV format.
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="/PurchaseOrder/PurchaseOrderDetail/PurchaseOrderLine">
			...
			<xsl:value-of select="substring($sProductDesc,1,40)"/-->
			<xsl:text>,</xsl:text>
			<!--xsl:value-of select="substring($sProductDesc,41,40)"/-->
			<xsl:text>,</xsl:text>

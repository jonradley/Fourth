...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="PurchaseOrder">
		...
		<xsl:if test="contains(TradeSimpleHeader/RecipientsBranchReference,',')">
			<xsl:text>"</xsl:text>
		</xsl:if>
		<xsl:value-of select="user:msEscapeQuotes(string(TradeSimpleHeader/RecipientsBranchReference))"/>
		<xsl:if test="contains(TradeSimpleHeader/RecipientsBranchReference,',')">
			<xsl:text>"</xsl:text>
		</xsl:if>
		<xsl:text>&#13;&#10;</xsl:text>

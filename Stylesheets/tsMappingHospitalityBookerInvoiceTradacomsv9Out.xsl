...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:js="http://www.abs-ltd.com/dummynamespaces/javascript" xmlns:vb="http://www.abs-ltd.com/dummynamespaces/vbscript" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template match="/Invoice">
		...
		<xsl:text>5011295000003</xsl:text>
		<xsl:text>:</xsl:text>
		<xsl:text>Booker Cash &amp; Carry Ltd</xsl:text>
		<xsl:text>+</xsl:text>
		...
		<xsl:choose>
			<xsl:when test="InvoiceHeader/Buyer/BuyersLocationID/GLN = '5011295000016'">
				<xsl:text>5011295000003</xsl:text>
			</xsl:when>
		...
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine1),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine2),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine3),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/Buyer/BuyersAddress/AddressLine4),35)"/>
		<xsl:text>:</xsl:text>
		...
		<xsl:text>+</xsl:text>
		<!-- truncate to 35 CADD 1-4 = 3032 = AN..35-->
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine1),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine2),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine3),35)"/>
		<xsl:text>:</xsl:text>
		<xsl:value-of select="js:msSafeText(string(InvoiceHeader/ShipTo/ShipToAddress/AddressLine4),35)"/>
		<xsl:text>:</xsl:text>
		<!-- truncate to 8 (just in case) CADD 5 = 3033 = AN..8-->
		...
		<xsl:for-each select="InvoiceDetail/InvoiceLine[generate-id() = generate-id(key('keyLinesByPO',concat('¬',PurchaseOrderReferences/PurchaseOrderReference))[1])]">
			...
			<xsl:for-each select="key('keyLinesByPO',$POReference)[generate-id() = generate-id(key('keyLinesByPOAndDN',concat($POReference,'::',concat('¬',DeliveryNoteReferences/DeliveryNoteReference)))[1])]">
				...
				<xsl:for-each select="key('keyLinesByPOAndDN',concat($POReference,'::',$DNReference))">
					...
					<xsl:text>+</xsl:text>
					<xsl:variable name="UOM" select="InvoicedQuantity/@UnitOfMeasure"/>
					<xsl:choose>
						<xsl:when test="$UOM ='KG'">

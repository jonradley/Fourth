...
Name			| Date			| Change
**********************************************************************
R Cambridge	| 19/12/2008	| 1556 remove checksum from 6 digitsite codes (branched from tsMappingHospitalityInvoiceTradacomsBatch_Catchweights.xsl)
**********************************************************************
				|             	|	
**********************************************************************
				|             	|
...
<BatchRoot>
	...
	<xsl:template match="SendersCodeForRecipient | ShipToLocationID/SuppliersCode">
		<xsl:copy>
			<xsl:choose>
				<xsl:when test="string-length(.) = 6">
					<!-- 6 digit codes are 5 didgit codes with a checksum -->

...
<BatchRoot>
	...
	<!--Document type no change to credit note (87)-->
	<xsl:template match="@DocumentTypeNo">
		<xsl:attribute name="{name()}">
			<xsl:choose>
				<xsl:when test="../@DocumentTypeNo = '0700' ">87</xsl:when>
	...
	<xsl:template match="BatchHeader/DocumentTotalExclVAT |
						...
						VATSubTotal/DocumentTotalExclVATAtRate |
						VATSubTotal/SettlementTotalExclVATAtRate |
						VATSubTotal/VATAmountAtRate |
						VATSubTotal/DocumentTotalInclVATAtRate |
						VATSubTotal/SettlementTotalInclVATAtRate |
						VATSubTotal/DiscountedLinesTotalExclVATAtRate |
	...
	<!-- Decoding the VAT code-->
	<xsl:template match="@VATCode">
		<xsl:attribute name="{name()}">
			<xsl:choose>
				<xsl:when test="@VATRate = '0000' ">Z</xsl:when>

...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="tempLines">
		<xsl:for-each select="//CreditNoteLine">
			...
			<xsl:if test="(format-number(number(CreditedQuantity) * number(UnitValueExclVAT), '#.00') &gt; format-number(number(LineValueExclVAT) + 0.01, '#.00')) or (format-number(number(CreditedQuantity) * number(UnitValueExclVAT), '#.00') &lt; format-number(number(LineValueExclVAT) - 0.01, '#.00'))">

...
******************************************************************************************
 Overview
 This XSL file is used to transform XML for a Moto Products vs Outlets Ordered and Receipted Detail Report into a CSV format
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 	
	...
	<msxsl:script language="JScript" implements-prefix="script"><![CDATA[ 
		...
		var productReceiptedTotal = new Array();
		...
		function headerLine(buyerNames, buyerIDs)
		{
			...
			for(i=1;i<buyerNames.length;i++)
			{
				...
				if (found == 0)
				{
					headerLine += buyerNames(i).text + ' Quantity Ordered, ' + buyerNames(i).text + ' Quantity Receipted, ' ;
		...
		' Alterations   	 : 
		'========================================================================================*/	
		function productLine(productCode, buyerID, ordered, receipted, description, packSize, category, subCategory)
		{
			...
			quantitiesStore(productCodeIndex, buyerID(0).text, ordered(0).text, receipted(0).text)	
			...
			// keep track of the ordered and receipted totals
		return 	totalsStore(productCodeIndex, ordered(0).text, receipted(0).text)
		...
		/*=========================================================================================
		' Routine       	 : totalsStore
		' Description 	 : Keeps a running total of ordered/receipted for each productCode
		...
		' Alterations   	 : 
		'========================================================================================*/
		function totalsStore(productCodeIndex, ordered, receipted)
		{
			var currentOrderedTotal = 0;
			var currentReceiptedTotal = 0;
			
			// we keep a running total for ordered and receipted, all other details are simply stored - they will b ethe same for the same product code
			currentOrderedTotal = productOrderedTotal[productCodeIndex];
			currentReceiptedTotal = productReceiptedTotal[productCodeIndex];
			...
			if (isNaN(currentReceiptedTotal))
				productReceiptedTotal[productCodeIndex] = receipted;
			else	
				productReceiptedTotal[productCodeIndex] = parseInt(currentReceiptedTotal) + parseInt(receipted);
		...
		' Alterations   	 : 
		'========================================================================================*/			
		function detailsStore(productCodeIndex, description, packSize, category, subCategory, ordered, receipted)
		{
		...
		' Description 	 : Stores unit ordered/receipted amounts in a module level array
		' Inputs          	 : The index to use in the array, the buyer identifier and the quantities ordered and receipted
		' Outputs       	 : None
		' Returns       	 : None
		' Author       		 : Steve Hewitt, 28/03/2006
		' Alterations   	 : 
		'========================================================================================*/		
		function quantitiesStore(productCodeIndex, buyerID, ordered, receipted)
		{
			var orderedPosition = 0;
			var receiptedPosition = 0;
			var buyerIndex = 0;
			...
			// The position in the productLine array will be buyerIndex * 2 (ordered) and buyerIndex * 2 + 1 (receipted)
			orderedPosition = buyerIndex * 2;
			receiptedPosition = orderedPosition + 1;
			...
			productQuantities[receiptedPosition] =  receipted;
		...
		{	
			...
			for (i=0;i<productCodes.length;i++)
			{
				result += productCodes[i] + ',' + productDetails[i] + ',' + productOrderedTotal[i] + ',' + productReceiptedTotal[i] + ',' + productLines[i]; 
				result += '\n'
			}	
			return result;
		}	

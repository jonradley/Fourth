	...
	<xsl:template match="/PurchaseOrder">
		...
		<xsl:for-each select="PurchaseOrderDetail/PurchaseOrderLine">
			...
			<xsl:text>OLD=</xsl:text>
				...
				<xsl:if test="string-length(ProductID/SuppliersProductCode)!=13 and string-length(translate(ProductID/SuppliersProductCode,'0123456789',''))!=0">
					<!-- Only put SuppliersProductCode in second sub field if it's not an EAN number -->
					<xsl:text>:</xsl:text>
				</xsl:if>

...
tsValidateBibendumInvoice.xsl
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Name		: tsValidateHospitalityInvoice.xsl
Description	: Validates hospitality invoices
Author		: A Sheppard
Date		: 04/03/2004
Alterations	: A Sheppard, 15/10/2004 - Upgraded for new schemas
			  A Sheppard, 17/12/2004. Add tolerances
...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="tempLines">
		<xsl:for-each select="//InvoiceLine">
			...
			<xsl:if test="(format-number(number(InvoicedQuantity) * number(UnitValueExclVAT), '#.00') &gt; format-number(number(LineValueExclVAT) + 0.01, '#.00')) or (format-number(number(InvoicedQuantity) * number(UnitValueExclVAT), '#.00') &lt; format-number(number(LineValueExclVAT) - 0.01, '#.00'))">
	...
	<xsl:template name="tempVATSubtotals">
		<xsl:for-each select="//VATSubTotal">
			...
			<xsl:variable name="Tolerance"><xsl:value-of select="0.01 * (count(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]) + count(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00') and LineDiscountValue and number(LineDiscountValue) != 0]))"/></xsl:variable>
			...
			<xsl:if test="(format-number(sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineValueExclVAT) - sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineDiscountValue), '#.00') &gt; format-number(number(DiscountedLinesTotalExclVATAtRate) + $Tolerance, '#.00')) or (format-number(sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineValueExclVAT) - sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineDiscountValue), '#.00') &lt; format-number(number(DiscountedLinesTotalExclVATAtRate) - $Tolerance, '#.00'))">
				<xsl:element name="Error">
	...
	<xsl:template name="tempTotals">
		<xsl:variable name="Tolerance"><xsl:value-of select="0.01 * (count(//InvoiceLine) + count(//InvoiceLine[LineDiscountValue and number(LineDiscountValue) != 0]))"/></xsl:variable>
		<!--Check NumberOfLines-->

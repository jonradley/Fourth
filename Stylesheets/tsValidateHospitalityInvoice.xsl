...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="tempVATSubtotals">
		<xsl:for-each select="//VATSubTotal">
			...
			<xsl:if test="(format-number(sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineValueExclVAT) - sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineDiscountValue), '#.00') &gt; format-number(number(DiscountedLinesTotalExclVATAtRate) + $Tolerance, '#.00')) or (format-number(sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineValueExclVAT) - sum(//InvoiceLine[VATCode=$VATCode and format-number(VATRate, '#.00')=format-number($VATRate, '#.00')]/LineDiscountValue), '#.00') &lt; format-number(number(DiscountedLinesTotalExclVAT) - $Tolerance, '#.00'))">
				<xsl:element name="Error">

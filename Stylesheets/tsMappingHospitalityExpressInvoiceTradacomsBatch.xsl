...
<BatchRoot>
	...
	<!-- EXP InvoicedQuantity is now handled within ProductDescription template -->
	<xsl:template match="InvoiceLine/InvoicedQuantity"/>
	<!-- EXP replaced entire function - INVOIC-ILD-QTYI (InvoiceLine/InvoicedQuantity) or QTYI2 (for VAL/KG lines) need to be multiplied by -1 if (InvoiceLine/ProductID/GTIN) is NOT blank -->
	<xsl:template match="InvoiceLine/ProductDescription">
		<xsl:variable name="QTYI1" select="string(../InvoicedQuantity)"/>
		<xsl:variable name="QTYI2" select="../Measure/TotalMeasure"/>
		<xsl:variable name="QTYI3" select="string(../Measure/TotalMeasureIndicator)"/>
		<xsl:variable name="QTYI3isEA" select="$QTYI3 != 'KG' and $QTYI3 != 'VAL'"/>
		<!--Parent of ProductDescription is InvoiceLine-->
		<xsl:variable name="GTINpresent" select="string-length(../ProductID/GTIN) &gt; 0"/>
		<!-- Make a copy of ProductDescription and then do the following element -->
		<xsl:copy>
			<xsl:value-of select="."/>
		</xsl:copy>
		<!-- Now do the following element, which is the one we are interested in -->
		<xsl:element name="InvoicedQuantity">
			<xsl:choose>
				<xsl:when test="$QTYI3isEA">
					<!-- This is an EA type line, use QTYI1( 0 implied D.P) -->
					<xsl:choose>
						<xsl:when test="$GTINpresent">
							<!--ILD-CRLI is not blank, multiply by -1-->
							<xsl:if test="string(number($QTYI1)) != 'NaN'">
								<xsl:value-of select="$QTYI1 * -1"/>
							</xsl:if>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="string(number($QTYI1)) != 'NaN'">
								<xsl:value-of select="$QTYI1"/>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<!-- This is a KG or VAL type line, use QTYI2 - 3 implied D.P. -->
					<xsl:choose>
						<xsl:when test="$GTINpresent" >
							<!--ILD-CRLI is not blank, multiply by -1-->
							<xsl:if test="string(number($QTYI2)) != 'NaN'">
								<xsl:value-of select="format-number(($QTYI2 * -1) div 1000.0, '0.00#')"/>
							</xsl:if>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="string(number($QTYI2)) != 'NaN'">
								<xsl:value-of select="format-number(($QTYI2) div 1000.0, '0.00#')"/>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>

...
**********************************************************************
A Barber		|	15/08/2013	|	6903: Branch of standard map for WYBC for bill to name for ISS HED.
**********************************************************************
-->
...
<BatchRoot>
	...
	<!-- Set buyer name to first line of address if ISS HED customer-->
	<xsl:template match="BuyersName">
		<xsl:choose>
			<xsl:when test="../BuyersLocationID/SuppliersCode='5060166761103'">
				<BuyersName>
					<xsl:value-of select="../BuyersAddress/AddressLine1"/>
				</BuyersName>
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy>

...
Name			| Date			| Change
**********************************************************************
S Hussain		| 2013-05-14	| Created a common stylesheets with generic functionalities by Matthew Clark
*******************************************************************-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:user="http://mycompany.com/mynamespace" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="#default xsl msxsl user">	
	...
	<xsl:variable name="CustomerFlag">
		<xsl:variable name="accountCode">
			<xsl:choose>
				...
				<xsl:when test="//ShipTo/SellerAssigned">
					<xsl:value-of  select="string(//ShipTo/SellerAssigned)"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="$accountCode = '7002'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7023'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7031'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7036'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7409'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7423'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7424'"><xsl:value-of select="$B_P"/></xsl:when>
			<xsl:when test="$accountCode = '7425'"><xsl:value-of select="$B_P"/></xsl:when>

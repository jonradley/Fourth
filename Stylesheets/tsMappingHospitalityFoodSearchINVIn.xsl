...
<!--***************************************************************************************
 $Header: $ $NoKeywords: $
 Overview 
  XSL Inbound Invoice mapper
  Hospitality inbound CSV to iXML format.
...
******************************************************************************************
 04/02/2015  	| M Dimant	| 7861: Created
******************************************************************************************
...
<BatchRoot>
	...
	<xsl:template match="TradeSimpleHeader">
		<xsl:copy>
			<!-- FSW will send SCR and SBR in ShipToName separated by a slash -->
			
				<xsl:choose>
					<!-- First check if a slash is present -->
					<xsl:when test="contains(../InvoiceHeader/ShipTo/ShipToName,'/')">
						<!-- If so, set everything before the slash as SCR -->	
						<xsl:element name="SendersCodeForRecipient">
							<xsl:value-of select="substring-before(../InvoiceHeader/ShipTo/ShipToName,'/')"/>	
						</xsl:element>
						<!-- And use everything after the slash as SBR -->
						<xsl:element name="SendersBranchReference">
							<xsl:value-of select="substring-after(../InvoiceHeader/ShipTo/ShipToName,'/')"/>
						</xsl:element>
					</xsl:when>
					<xsl:otherwise>
						<!-- If no slash is present in the string, just pass in the entire value from ShipToName -->	
						<xsl:element name="SendersCodeForRecipient">
			...
			<xsl:element name="TestFlag">
				...
				<xsl:choose>
					<xsl:when test="string(TestFlag) = 'N'">

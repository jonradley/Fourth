...
**********************************************************************
R Cambridge	| 2013-06-28		| FB 6702 - Added Validation on WebOrderNo Node to Set Order Confirmation Status to Rejected.
**********************************************************************
				|						|				
**********************************************************************
				|						|				
...
/BatchRoot/ResponseDocument/soap:Envelope/soap:Body/*/*/*[local-name()='WebOrderNo']"/>
		<xsl:variable name="orderStatus" select="
/BatchRoot/ResponseDocument/soap:Envelope/soap:Body/*/*/*[local-name()='Status']"/>
		...
		<xsl:variable name="lineAction">
			<xsl:choose>
				<!--If WebOrderNo = 0, Then something has gone wrong at Bread Factory End and they have not accepted the Purchase Order, So Set Status as Rejected-->
				<xsl:when test="$orderNumber = '0'">Rejected</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
		...
		<xsl:variable name="confirmedQuantity">
			<xsl:choose>
				<xsl:when test="$orderNumber = '0'">0</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
		...
		<PurchaseOrderConfirmationLine>
			<xsl:attribute name="LineStatus"><xsl:value-of select="$lineAction"/></xsl:attribute>
			...
			<ConfirmedQuantity>
				<xsl:attribute name="UnitOfMeasure"><xsl:value-of select="$orderLine/OrderedQuantity/@UnitOfMeasure"/></xsl:attribute>

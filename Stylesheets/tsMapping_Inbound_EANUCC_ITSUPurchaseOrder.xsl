				...
				<PurchaseOrderDetail>
					<xsl:for-each select="/Order/OrderDetails">
						<PurchaseOrderLine>
							...
							<OrderedQuantity>
								<xsl:attribute name="UnitOfMeasure"><xsl:call-template name="decodeUoM"><xsl:with-param name="sInput"><xsl:value-of select="RequestedQuantity/@unitCode"/></xsl:with-param></xsl:call-template><!--xsl:value-of select="RequestedQuantity/@unitCode"/--></xsl:attribute>

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/BatchRoot[DeliveryNote]">
		...
		<xsl:text>STX=</xsl:text>
			...
			<xsl:text>:</xsl:text>
			<xsl:value-of select="js:msSafeText(string(DeliveryNote/DeliveryNoteHeader/Supplier/SuppliersName), 35)"/>
			<xsl:text>+</xsl:text>			
			<xsl:value-of select="DeliveryNote/DeliveryNoteHeader/Buyer/BuyersLocationID/GLN"/>
			<xsl:text>:</xsl:text>
			<xsl:value-of select="js:msSafeText(string(DeliveryNote/DeliveryNoteHeader/Buyer/BuyersName), 35)"/>
			<xsl:text>+</xsl:text>
		...
		<xsl:text>SDT=</xsl:text>
			<xsl:choose>
				<xsl:when test="DeliveryNote/DeliveryNoteHeader/Supplier/SuppliersLocationID/GLN != ''">
					<xsl:value-of select="DeliveryNote/DeliveryNoteHeader/Supplier/SuppliersLocationID/GLN"/>
				</xsl:when>
				<xsl:otherwise>					
					<xsl:value-of select="js:msSafeText(string(DeliveryNote/DeliveryNoteHeader/Supplier/SuppliersLocationID/SuppliersCode),17)"/>			
				</xsl:otherwise>
			</xsl:choose>		
		<xsl:value-of select="$sRecordSep"/>
		...
		<xsl:text>CDT=</xsl:text>
			<xsl:value-of select="DeliveryNote/DeliveryNoteHeader/Buyer/BuyersLocationID/GLN"/>		
			<xsl:text>:</xsl:text>		
			<xsl:value-of select="js:msSafeText(string(DeliveryNote/DeliveryNoteHeader/Supplier/SuppliersLocationID/BuyersCode),17)"/>		
		<xsl:value-of select="$sRecordSep"/>
		...
		<xsl:text>MHD=</xsl:text>
			<xsl:value-of select="format-number(count(/BatchRoot/DeliveryNote) + 2,'0')"/>
			<xsl:text>+</xsl:text>

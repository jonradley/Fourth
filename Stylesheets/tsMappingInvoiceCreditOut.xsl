...
******************************************************************************************
 ...
 Maps internal invoices and credits into a csv format for Crerar Company.
...
******************************************************************************************
 23/06/2008	   | S Dubey  	   | Created Module
******************************************************************************************
-->
		...
		<xsl:for-each select="(CreditNoteDetail/CreditNoteLine | InvoiceDetail/InvoiceLine)[generate-id() = generate-id(key('keyLinesByAccount',LineExtraData/AccountCode)[1])]">
			...
			<xsl:for-each select="key('keyLinesByAccount',$AccountCode)[generate-id() = generate-id(key('keyLinesByAccountAndVAT',concat($AccountCode,'::',VATCode))[1])]">
				...
				<xsl:text>,</xsl:text>
					...
					<xsl:variable name="Sumvalue"  select="format-number(sum(//InvoiceLine[VATCode = $VATCode and LineExtraData/AccountCode = $AccountCode]/LineValueExclVAT) + sum(//CreditNoteLine[VATCode = $VATCode and LineExtraData/AccountCode = $AccountCode]/LineValueExclVAT),'0.00')"/>
				...
				<xsl:value-of select="format-number($Sumvalue* ($VatRate div 100), '0.00')" />
				<xsl:text>,</xsl:text>

				<xsl:text>CrerarReference</xsl:text>
				<xsl:text>,</xsl:text>
				
				<xsl:value-of select="/Invoice/InvoiceHeader/InvoiceReferences/InvoiceReference" />
				<xsl:text>,</xsl:text>
				
				<xsl:text>CrerarReference</xsl:text><xsl:text>/</xsl:text>

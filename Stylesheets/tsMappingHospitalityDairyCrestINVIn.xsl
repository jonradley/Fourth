...
Name			| Date				| Change
**********************************************************************
M Dimant	| 24/05/2011		| Map product code from CustomersConceptProductCode when present.
...
<BatchRoot>
	...
	<!-- Pick up product code from CustomersConceptProductCode when it is present. This is actually the correct Dairy Crest product code,
	and overwrites the agent's product code in SuppliersProductCode -->
	<xsl:template match="ProductID">
	<ProductID>
		<GTIN><xsl:value-of select="GTIN"/></GTIN>
		<SuppliersProductCode>
			<xsl:choose>
				<xsl:when test="../LineExtraData/CustomersConceptProductCode">
					<xsl:value-of select="../LineExtraData/CustomersConceptProductCode"/>
				</xsl:when>
				<xsl:otherwise>

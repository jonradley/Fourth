...
<!--***************************************************************************************
 ...
 Overview 
  ...
  Hospitality iXML to CSV Outbound format with delivery instructions.
...
******************************************************************************************
 21/11/2103	| A Barber		| 7445 Created new map based on Urbium CSV map with delivery instructions.
***************************************************************************************-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="PurchaseOrder">
		...
		<xsl:if test="contains(user:msEscapeQuotes(substring(PurchaseOrderHeader/OrderedDeliveryDetails/SpecialDeliveryInstructions,1,200)),',')">
			<xsl:text>"</xsl:text>
		</xsl:if>
		<xsl:value-of select="user:msEscapeQuotes(substring(PurchaseOrderHeader/OrderedDeliveryDetails/SpecialDeliveryInstructions,1,200))"/>
		<xsl:if test="contains(user:msEscapeQuotes(substring(PurchaseOrderHeader/OrderedDeliveryDetails/SpecialDeliveryInstructions,1,200)),',')">
			<xsl:text>"</xsl:text>
		</xsl:if>

...
**********************************************************************
R cambridge	| 03/05/2007	| 1047 branched for Key Lekkerland (2 implied dpl in VAT rates, 
...
<BatchRoot>
	...
	<!--Add any attribute XPath whose value needs to be converted from implicit 2 D.P to explicit 2 D.P. -->
	<xsl:template match="VATSubTotal/@VATRate">
		<xsl:attribute name="{name()}">
			<xsl:value-of select="format-number(. div 100.0, '0.00')"/>
		</xsl:attribute>
	</xsl:template>
	<!--Add any element XPath whose value needs to be converted from implicit 2 D.P to explicit 2 D.P. -->

	...
	<xsl:template name="GetHeaderNarrative">
		...
		<xsl:choose>
			...
			<xsl:when test="$HeaderCode = 1">
				<xsl:text>Invalid Order Indicator, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 002 - Invalid Ship-to customer -->
			<xsl:when test="$HeaderCode = 2">
				<xsl:text>Invalid Ship-to customer, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 003 - Invalid Sold-to customer -->
			<xsl:when test="$HeaderCode = 3">
				<xsl:text>Invalid Sold-to customer, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 004 - Invalid Order Type -->
			<xsl:when test="$HeaderCode = 4">
				<xsl:text>Invalid Order Type, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			...
			<xsl:when test="$HeaderCode = 6">
				<xsl:text>Delivery Plant not found, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 007 - Invalid Delivery Block indicator -->
			<xsl:when test="$HeaderCode = 7">
				<xsl:text>Invalid Delivery Block indicator, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 010 - Invalid Delivery From Time -->
			<xsl:when test="$HeaderCode = 10">
				<xsl:text>Invalid Delivery From Time, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 012 - Invalid Item Line Count -->
			<xsl:when test="$HeaderCode = 12">
				<xsl:text>Invalid Item Line Count, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 014 - Delivery Created - Amend/Cancel not allowed -->
			<xsl:when test="$HeaderCode = 14">
				<xsl:text>Delivery Created - Amend/Cancel not allowed, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 015 - Partner Order Number already exists (New) -->
			<xsl:when test="$HeaderCode = 15">
				<xsl:text>Partner Order Number already exists (New), please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 016 - Plant on amend different from original order -->
			<xsl:when test="$HeaderCode = 16">
				<xsl:text>Plant on amend different from original order, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 021 - Partner Order Number not found (Amend) -->
			<xsl:when test="$HeaderCode = 21">
				<xsl:text>Partner Order Number not found (Amend), please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 024 - VA01 Failure - See text on ZORDH -->
			<xsl:when test="$HeaderCode = 24">
				<xsl:text>VA01 Failure, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 026 - VA02 Failure - See text on ZORDH -->
			<xsl:when test="$HeaderCode = 26">
				<xsl:text>VA02 Failure, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			<!-- 027 - Sold-to not linked to interface partner -->
			<xsl:when test="$HeaderCode = 27">
				<xsl:text>Sold-to not linked to interface partner, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 028 - Sales org. record not found for customer -->
			<xsl:when test="$HeaderCode = 28">
				<xsl:text>Sales org. record not found for customer, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
			...
			<xsl:when test="$HeaderCode = 32">
				<xsl:text>Invalid Ullage Return, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			...
			<xsl:when test="$HeaderCode = 35">
				<xsl:text>Invalid Ullage - destroyed on site material, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 036 - W&S emergency order received late -->
			<xsl:when test="$HeaderCode = 36">
				<xsl:text>W&amp;S emergency order received late, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 037 - Cancellation of W&S not allowed -->
			<xsl:when test="$HeaderCode = 37">
				<xsl:text>Cancellation of W&amp;S not allowed, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 038 - Order Block rejected - W&S sent -->
			<xsl:when test="$HeaderCode = 38">
				<xsl:text>Order Block rejected - W&amp;S sent, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 039 - W&S emergency not for next day -->
			<xsl:when test="$HeaderCode = 39">
				<xsl:text>W&amp;S emergency not for next day, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 040 - Invalid SSL reason code -->
			<xsl:when test="$HeaderCode = 40">
				<xsl:text>Invalid SSL reason code, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			...
			<xsl:when test="$HeaderCode = 42">
				<xsl:text>Emergency delivery not next day, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 043 - Cellar Tank > 2 months in past -->
			<xsl:when test="$HeaderCode = 43">
				<xsl:text>Cellar Tank > 2 months in past, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 044 - Cellar Tank > 2 weeks in future -->
			<xsl:when test="$HeaderCode = 44">
				<xsl:text>Cellar Tank > 2 weeks in future, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 045 - WVL emergency unblock late - order deleted -->
			<xsl:when test="$HeaderCode = 45">
				<xsl:text>WVL emergency unblock late - order deleted, please contact ABS Helpdesk</xsl:text>
			</xsl:when>
			<!-- 099 - EDI Partner not known - ETS to issue alert -->
			<xsl:when test="$HeaderCode = 99">
				<xsl:text>EDI Partner not known - ETS to issue alert, please contact ABS Helpdesk</xsl:text>
			</xsl:when>			
		...
		<xsl:if test="$Narrative != ''">
			...
			<xsl:choose>
				...
				<xsl:when test="$LineCode = 101">
					<xsl:text>Invalid Order Line Type, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				...
				<xsl:when test="$LineCode = 106">
					<xsl:text>Invalid FOC item category, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 109 - Invalid sales/return indicator -->
				<xsl:when test="$LineCode = 109">
					<xsl:text>Invalid sales/return indicator, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				...
				<xsl:when test="$LineCode = 114">
					<xsl:text>Material not valid for Cellar Tank, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 115 - Invalid Sales of Container -->
				<xsl:when test="$LineCode = 115">
					<xsl:text>Invalid Sales of Container, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 116 - Material not an empty for a returns order -->
				<xsl:when test="$LineCode = 116">
					<xsl:text>Material not an empty for a returns order, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 117 - W&S item rejected - order received after cut-off -->
				<xsl:when test="$LineCode = 117">
					<xsl:text>W&amp;S item rejected - order received after cut-off, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 118 - Non W&S on W&S emergency order -->
				<xsl:when test="$LineCode = 118">
					<xsl:text>Non W&amp;S on W&amp;S emergency order, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 119 - W&S on non W&S order type -->
				<xsl:when test="$LineCode = 119">
					<xsl:text>W&amp;S on non W&amp;S order type, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 120 - W&S Amend - Already Sent to Waverley -->
				<xsl:when test="$LineCode = 120">
					<xsl:text>W&amp;S Amend - Already Sent to Waverley, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 121 - Unblock received after WVL cutoff, W&S item deleted -->
				<xsl:when test="$LineCode = 121">
					<xsl:text>Unblock received after WVL cutoff, W&amp;S item deleted, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				...
				<xsl:when test="$LineCode = 126">
					<xsl:text>WVL account number not found on ship-to acct, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				...
				<xsl:when test="$LineCode = 129">
					<xsl:text>Invalid customer material number, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 130 - Item cannot be amended - Del.Note created -->
				<xsl:when test="$LineCode = 130">
					<xsl:text>Item cannot be amended - Del.Note created, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
				<!-- 131 - W&S item rejected - No stock -->
				<xsl:when test="$LineCode = 131">
					<xsl:text>W&amp;S item rejected - No stock, please contact ABS Helpdesk</xsl:text>
				</xsl:when>
			</xsl:choose>

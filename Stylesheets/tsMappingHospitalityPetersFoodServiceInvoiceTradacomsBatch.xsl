...
<BatchRoot>
	...
	<!-- Van sales mean there may not be a PO reference -->
	<xsl:template match="PurchaseOrderReferences/PurchaseOrderReference">
		<PurchaseOrderReference>
			<xsl:choose>
				<xsl:when test=".='0000'">
					<xsl:value-of select="concat('20',substring(../PurchaseOrderDate,1,2),'-',substring(../PurchaseOrderDate,3,2),'-',substring(../PurchaseOrderDate,5,2))"/>
				</xsl:when>

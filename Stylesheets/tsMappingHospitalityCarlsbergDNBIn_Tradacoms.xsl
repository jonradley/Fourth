...
**********************************************************************
A Barber		| 21/07/2011	| Updated map for CUK to group product lines by order
				  / product code in order to handle batch splits.
*******************************************************************-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:output method="xml" encoding="utf-8"/>

	<!--Define key to be used for finding distinct line information. -->
	<xsl:key name="keyLinesByProduct" match="DeliveryNoteDetail/DeliveryNoteLine" use="concat(ProductID/SuppliersProductCode,'+',../../DeliveryNoteHeader/PurchaseOrderReferences/PurchaseOrderReference)"/>	
						...
						<BatchDocument>
							...
							<DeliveryNote>
								...
								<DeliveryNoteDetail>
									...
									<xsl:for-each select="(DeliveryNoteDetail/DeliveryNoteLine)[generate-id() = generate-id(key('keyLinesByProduct',concat(ProductID/SuppliersProductCode,'+',../../DeliveryNoteHeader/PurchaseOrderReferences/PurchaseOrderReference))[1])]">
									
									<xsl:variable name="ProductCode" select="concat(ProductID/SuppliersProductCode,'+',../../DeliveryNoteHeader/PurchaseOrderReferences/PurchaseOrderReference)"/>
										...
										<DeliveryNoteLine>
											...
											<xsl:for-each select="OrderedQuantity[1]">
												<OrderedQuantity RecordPos="DLD=" LPos="6" SFPos="2">
													...
													<xsl:value-of select="sum(//DeliveryNoteLine[concat(ProductID/SuppliersProductCode,'+',../../DeliveryNoteHeader/PurchaseOrderReferences/PurchaseOrderReference) = $ProductCode]/OrderedQuantity)"/>
											...
											<DespatchedQuantity UnitOfMeasure="EA">		
												...
												<xsl:value-of select="sum(//DeliveryNoteLine[concat(ProductID/SuppliersProductCode,'+',../../DeliveryNoteHeader/PurchaseOrderReferences/PurchaseOrderReference) = $ProductCode]/DespatchedQuantity)"/>

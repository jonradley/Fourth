...
==========================================================================================
 17/10/2013	| M Dimant           	| 7229: Created Greencore outbound tradacoms order mapper.
=======================================================================================-->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/PurchaseOrder">
		...
		<xsl:variable name="FourDigitFGN">
			<!-- Restrict the FGN to 4 digits (tradacoms standard). We take the 4 right hand digits -->		
			<xsl:variable name="FGN" select="format-number(PurchaseOrderHeader/FileGenerationNumber,'0000')"/>			
			<xsl:value-of select="substring($FGN, string-length($FGN)-3)"/>
		</xsl:variable>
		...
		<xsl:text>MHD=</xsl:text>	
			<xsl:value-of select="format-number(count(/PurchaseOrder) + 2,'0')"/>
			<xsl:text>+ORDTLR:9</xsl:text>		
		<xsl:value-of select="$sRecordSep"/>
		
		<xsl:text>OFT=</xsl:text>	
			<xsl:value-of select="format-number(count(/PurchaseOrder),'0')"/>			
		<xsl:value-of select="$sRecordSep"/>

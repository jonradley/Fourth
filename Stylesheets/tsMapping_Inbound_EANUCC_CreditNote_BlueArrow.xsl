							...
							<CreditNoteDetail>
								<xsl:for-each select="/CreditNote/CreditItem">
									...
									<CreditNoteLine>
										...
										<ProductDescription>
											<xsl:value-of select="/CreditNote/CSGExtension/WorkerName"/>
											<xsl:text>, </xsl:text>
											<xsl:value-of select="/CreditNote/CSGExtension/JobDesc"/>
											<xsl:text>, w/c </xsl:text>
											<xsl:value-of select="script:getWeekBeingBilled(substring-before(/CreditNote/TaxPointDateTime, 'T'))"/>
										</ProductDescription>

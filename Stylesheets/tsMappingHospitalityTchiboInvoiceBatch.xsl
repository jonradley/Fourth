								...
								<InvoiceDetail>
									<xsl:for-each select="Invoice/InvoiceDetail/InvoiceLine">
										<InvoiceLine>
											...
											<VATRate>
												<xsl:value-of select="format-number(number(../../InvoiceTrailer/VATSubTotals/VATSubTotal[@VATCode  = current()/VATCode]/VATAmountAtRate) div number(../../InvoiceTrailer/VATSubTotals/VATSubTotal[@VATCode  = current()/VATCode]/DocumentTotalExclVATAtRate) * 100,'0.0')"/>
											</VATRate>
										</InvoiceLine>	
									</xsl:for-each>								
								</InvoiceDetail>

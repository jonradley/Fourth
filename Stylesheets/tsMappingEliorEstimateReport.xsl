...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- this key creates an index to allow grouping of report lines by CompanyCode (column 3) -->
	<xsl:key name="keyByCompanyCode" match="LineDetails/LineDetail" use="Columns/Column[@ID='3']"/>
	<!-- this key creates an index to allow grouping of report lines by both CompanyCode and UnitMemberID (column 1) 
	note that the '::' literal is simply used as a convenient separator for the values that make up this 2 part compound key -->
	<xsl:key name="keyByCompanyCodeAndUnit" match="LineDetails/LineDetail" use="concat(Columns/Column[@ID='3'],'::',Columns/Column[@ID='1'])"/>
	<xsl:template match="/Report">
		...
		<Root>
			<!-- we want an output batch file for each distinct company code and then within each batch a distinct journal (GL entry) for each Unit -->
			<!-- the first loop will match the first report detail line in each set of lines grouped by CompanyCode -->
			<xsl:for-each select="LineDetails/LineDetail[generate-id() = generate-id(key('keyByCompanyCode',Columns/Column[@ID='3'])[1])]">
				<xsl:sort select="Columns/Column[@ID='3']" data-type="text"/>
				<xsl:variable name="CompanyCode" select="Columns/Column[@ID='3']"/>
				<CyBizXML version="2.0">
					<Body>
						<BatchEntry>
							...
							<Comments/>
							<GeneralLedgers>
								<!-- now, given we can find all detail lines for the current Company Code, loop through and match the first line for each unique Unit -->
								<xsl:for-each select="key('keyByCompanyCode',$CompanyCode)[generate-id() = generate-id(key('keyByCompanyCodeAndUnit',concat($CompanyCode,'::',Columns/Column[@ID='1']))[1])]">
									<!-- sort by Unit and then by the IsEstimateControl flag -->
									<xsl:sort select="Columns/Column[@ID='1']" data-type="text"/>
									<xsl:sort select="Columns/Column[@ID='5']" data-type="text"/>
									<xsl:variable name="UnitMemberID" select="Columns/Column[@ID='1']"/>
									<GeneralLedger>
										...
										<CostCentreCode><xsl:value-of select="Columns/Column[@ID='2']"/></CostCentreCode>
										...
										<xsl:for-each select="key('keyByCompanyCodeAndUnit',concat($CompanyCode,'::',$UnitMemberID))">
											<GLLine>
											<xsl:choose>
												<xsl:when test="Columns/Column[@ID='5'] = 0">
													...
													<xsl:attribute name="AccountCode"><xsl:value-of select="Columns/Column[@ID='6']"/></xsl:attribute>
													<!-- use the document type to determine whether this is a debit (invoice) or a credit (credit) amount -->
													<xsl:choose>
														<xsl:when test="Columns/Column[@ID='4'] = 86">
															<xsl:attribute name="DebitAmount"><xsl:value-of select="Columns/Column[@ID='7']"/></xsl:attribute>
														...
														<xsl:otherwise>
															...
															<xsl:attribute name="CreditAmount"><xsl:value-of select="Columns/Column[@ID='7']"/></xsl:attribute>
														</xsl:otherwise>
													</xsl:choose>
												</xsl:when>
												<xsl:otherwise>
													...
													<xsl:choose>
														<xsl:when test="Columns/Column[@ID='7'] > 0">
															<xsl:attribute name="DebitAmount"><xsl:value-of select="0"/></xsl:attribute>
															<xsl:attribute name="CreditAmount"><xsl:value-of select="Columns/Column[@ID='7']"/></xsl:attribute>
														</xsl:when>
														<xsl:otherwise>
															<xsl:attribute name="DebitAmount"><xsl:value-of select="Columns/Column[@ID='7'] * -1"/></xsl:attribute>
															<xsl:attribute name="CreditAmount"><xsl:value-of select="0"/></xsl:attribute>
														</xsl:otherwise>
													</xsl:choose>
												</xsl:otherwise>
